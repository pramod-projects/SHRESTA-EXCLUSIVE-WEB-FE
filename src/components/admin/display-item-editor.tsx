"use client";

import Image from "next/image";
import { useState } from "react";
import { assignAssetToDisplayItemAction } from "@/app/admin/actions";
import { AdminActionForm, AdminSubmitButton } from "@/components/admin/admin-action-form";
import { DirectMediaInput } from "@/components/admin/direct-media-input";
import type { AssetResponse } from "@/features/admin/admin-api";
import type { MediaAsset } from "@/features/storefront/storefront-home";

interface Props {
  itemKey: string;
  title: string;
  sectionLabel: string;
  currentImage: MediaAsset | null;
  availableAssets: AssetResponse[];
  idempotencyKey: string;
  allowUpload?: boolean;
  logoAsset?: MediaAsset | null;
}

type SelectableImage = {
  assetKey: string;
  assetUrl: string;
  altText: string;
  widthPx: number;
  heightPx: number;
  isLogo?: boolean;
};

export function DisplayItemEditor({ itemKey, title, sectionLabel, currentImage, availableAssets, idempotencyKey, allowUpload = false, logoAsset }: Props) {
  const [mode, setMode] = useState<"grid" | "upload">("grid");
  const [selected, setSelected] = useState<SelectableImage | null>(null);
  const [open, setOpen] = useState(false);
  const imageAssets: SelectableImage[] = availableAssets
    .filter((asset) => asset.contentType?.startsWith("image/"))
    .map((asset) => ({
      assetKey: asset.assetKey,
      assetUrl: asset.assetUrl,
      altText: asset.altText,
      widthPx: asset.widthPx,
      heightPx: asset.heightPx,
      isLogo: asset.assetKey === logoAsset?.assetKey
    }));
  if (logoAsset && !imageAssets.some((asset) => asset.assetKey === logoAsset.assetKey)) {
    imageAssets.unshift({
      assetKey: logoAsset.assetKey,
      assetUrl: logoAsset.url,
      altText: logoAsset.altText,
      widthPx: logoAsset.width,
      heightPx: logoAsset.height,
      isLogo: true
    });
  }

  return (
    <div className="rounded-lg border border-[var(--shresta-logo-border)] bg-[var(--shresta-logo-surface)] p-4">
      {/* Item header */}
      <div className="flex items-start gap-3">
        {currentImage ? (
          <Image
            alt={currentImage.altText}
            className="h-16 w-16 shrink-0 rounded-lg border border-[var(--shresta-logo-border)] object-cover"
            src={currentImage.url}
            unoptimized
            width={64}
            height={64}
          />
        ) : (
          <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-lg border border-dashed border-[var(--shresta-logo-border)] text-[10px] text-[var(--shresta-logo-muted)]">
            No image
          </div>
        )}
        <div className="min-w-0 flex-1">
          <span className="rounded-full bg-[rgba(212,175,55,0.1)] px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.1em] text-[var(--gold-400)]">
            {sectionLabel}
          </span>
          <p className="mt-1 truncate font-serif text-lg font-light text-[var(--shresta-logo-text)]">{title}</p>
          {currentImage && (
            <p className="mt-0.5 truncate text-[10px] text-[var(--shresta-logo-muted)]">{currentImage.assetKey}</p>
          )}
        </div>
        <button
          className="admin-button secondary shrink-0 text-xs"
          onClick={() => setOpen((v) => !v)}
          type="button"
        >
          {open ? "Cancel" : "Change Image"}
        </button>
      </div>

      {/* Change image panel */}
      {open && (
        <div className="mt-4 space-y-4">
          {/* Mode tabs */}
          <div className="flex gap-3 border-b border-[var(--shresta-logo-border)] pb-2">
            <button
              className={`text-sm font-medium ${mode === "grid" ? "text-[var(--gold-400)]" : "text-[var(--shresta-logo-muted)] hover:text-[var(--shresta-logo-text)]"}`}
              onClick={() => setMode("grid")}
              type="button"
            >
              Choose from library
            </button>
            {allowUpload ? (
              <button
                className={`text-sm font-medium ${mode === "upload" ? "text-[var(--gold-400)]" : "text-[var(--shresta-logo-muted)] hover:text-[var(--shresta-logo-text)]"}`}
                onClick={() => setMode("upload")}
                type="button"
              >
                Upload logo
              </button>
            ) : null}
          </div>

          <AdminActionForm action={assignAssetToDisplayItemAction}>
            <input name="idempotencyKey" type="hidden" value={idempotencyKey} />
            <input name="itemKey" type="hidden" value={itemKey} />

            {mode === "grid" ? (
              <>
                {/* Asset grid */}
                <div className="grid max-h-72 grid-cols-4 gap-2 overflow-y-auto pr-1 sm:grid-cols-6 md:grid-cols-8">
                  {imageAssets.map((asset) => (
                    <button
                      className={`group relative overflow-hidden rounded-lg border transition-all ${
                        selected?.assetKey === asset.assetKey
                          ? "border-[var(--gold-400)] ring-1 ring-[var(--gold-400)]"
                          : "border-[var(--shresta-logo-border)] hover:border-[var(--gold-600)]"
                      }`}
                      key={asset.assetKey}
                      onClick={() => setSelected(asset)}
                      title={asset.assetKey}
                      type="button"
                    >
                      <Image
                        alt={asset.altText}
                        className="aspect-square w-full object-cover"
                        src={asset.assetUrl}
                        unoptimized
                        width={120}
                        height={120}
                      />
                      {asset.isLogo ? (
                        <span className="absolute left-1 top-1 rounded bg-black/75 px-1.5 py-0.5 text-[9px] font-bold uppercase text-white">
                          Logo
                        </span>
                      ) : null}
                      {selected?.assetKey === asset.assetKey && (
                        <div className="absolute inset-0 flex items-center justify-center bg-[rgba(212,175,55,0.3)]">
                          <span className="text-lg text-[var(--shresta-logo-text)]">✓</span>
                        </div>
                      )}
                    </button>
                  ))}
                </div>

                {selected && (
                  <p className="mt-2 text-xs text-[var(--gold-600)]">
                    Selected: <strong>{selected.assetKey}</strong> · {selected.widthPx}×{selected.heightPx}
                  </p>
                )}

                {/* Hidden fields for selected asset */}
                <input name="selectedAssetKey" type="hidden" value={selected?.assetKey ?? ""} />

                <AdminSubmitButton
                  className="mt-3 w-full"
                  disabled={!selected}
                  label={selected ? `Apply "${selected.assetKey}"` : "Select an image above"}
                />
              </>
            ) : (
              <>
                <DirectMediaInput
                  accept="image/jpeg,image/png,image/webp"
                  fieldName="media"
                  label="Canonical display image"
                  mediaType="DISPLAY_IMAGE"
                  required
                  captureMediaId
                />
                <AdminSubmitButton className="mt-3 w-full" label="Upload & Apply" />
              </>
            )}
          </AdminActionForm>
        </div>
      )}
    </div>
  );
}
