...
export function BackendApiUnavailable({ frame = "page", surface }: BackendApiUnavailableProps) {
  const className = frame === "page"
    ? "flex min-h-screen items-center justify-center bg-[var(--shresta-logo-bg)] px-4 text-[var(--shresta-logo-text)]"
    : "flex min-h-[70vh] items-center justify-center px-4 text-[var(--shresta-logo-text)]";
  ...
  return (
    <div className={className}>
      <section className="max-w-xl rounded-lg border border-[var(--shresta-logo-border)] bg-[var(--shresta-logo-surface)] p-8 text-center shadow-[0_20px_70px_rgba(47,33,21,0.18)]">
        ...
        <h1 className="mt-4 font-serif text-4xl font-light text-[var(--shresta-logo-text)]">{copy.title}</h1>
