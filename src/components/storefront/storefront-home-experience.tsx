...
import { SUPPORT_EMAIL, SUPPORT_PHONE_DIAL, SUPPORT_PHONE_DISPLAY } from "@/lib/support-contact";
import type {
  FeaturedCollection,
  HeroSlide,
  ProductCard,
...
export type StorefrontChromeControls = {
  ...
  cartLineQuantity: (productId: string) => number;
...
export function StorefrontPageChrome({ home, children }: StorefrontPageChromeProps) {
  ...
  const { addItem, itemCount: cartCount, lines: cartLines, removeItem, updateQuantity } = useBrowserCart();
  const cartLineQuantityMap = new Map(cartLines.map((line) => [line.productId, line.quantity]));

  const controls: StorefrontChromeControls = {
    cartCount,
    wishlistKeys,
    cartLineQuantity: (productId) => cartLineQuantityMap.get(productId) ?? 0,
            ...
            return (
              <>
                {visibleBestsellers.length === 0 ? (
                  <div className="mt-8 border-y border-[var(--shresta-logo-border)] py-12 text-center">
                    <p className="font-serif text-2xl font-light text-[var(--shresta-logo-text)]">No products available</p>
                  </div>
                ) : (
...
function CustomerHeader({
  ...
  for (const product of home.bestsellers) {
  ...
  const collectionByTypeToken = new Map(
    home.featuredCollections.map((collection) => [collectionTypeToken(collection.slug), collection])
  );

  const featuredOrderedCollections = home.featuredCollections
    .map((collection) => {
      const token = collectionTypeToken(collection.slug);
      const matchedType = Array.from(productTypeCounts.keys()).find((productType) => collectionTypeToken(productType) === token);
      if (!matchedType) {
        return null;
      }
      return {
  ...
  const featuredTypeKeys = new Set(featuredOrderedCollections.map((entry) => entry.key));
  const unmatchedTypeCollections = Array.from(productTypeCounts.keys())
    .filter((productType) => !featuredTypeKeys.has(productType))
  ...
  const productTypeCollections = [...featuredOrderedCollections, ...unmatchedTypeCollections];
  const exploreLinks = [
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[var(--shresta-logo-border)] bg-[var(--shresta-logo-bg)] shadow-[0_10px_24px_rgba(47,33,21,0.12)]">
      <div className="container mx-auto px-3 sm:px-4">
        <div className="relative flex h-20 items-center justify-between gap-2 lg:h-[6.4rem] xl:h-[6.8rem]">
          <button
          ...
          <Link aria-label={`${home.brand.name} home`} className="flex min-w-0 items-center" href="/" prefetch={false}>
            <BrandLogo media={home.brand.logo} variant="header" />
          </Link>

          <nav className="hidden min-w-0 flex-1 items-center justify-center gap-2 px-4 lg:flex xl:gap-3 xl:px-6">
            {home.navigation.map((item) => (
              ...
              >
                <Link
                  className="flex items-center gap-1.5 px-3 py-2.5 text-base font-medium text-[var(--shresta-logo-muted)] transition-colors duration-300 hover:text-[var(--shresta-logo-text)] xl:px-4"
                  href={item.href}
                  prefetch={false}
                >
                  {item.label}
          ...
          <div className="flex shrink-0 items-center gap-0 sm:gap-1 lg:justify-end">
            <AnimatePresence>
            {searchOpen ? (
              <motion.form
                action="/products"
                animate={{ opacity: 1 }}
                className="hidden w-[9.5rem] lg:w-[10.5rem] xl:w-[12.5rem] md:block"
                exit={{ opacity: 0 }}
                initial={{ opacity: 0 }}
            ...
            <Link
              ...
              className="reference-icon-button !h-9 !w-9 relative hidden md:inline-flex lg:scale-110"
              href="/account"
              prefetch={false}
              title="Account"
            >
...
function BrandIntroVideo({ brandName, videoUrl }: { brandName: string; videoUrl: string }) {
  ...
  const frameCheckTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const [videoHelpMessage, setVideoHelpMessage] = useState<string | null>(null);
  const [videoHelpSteps, setVideoHelpSteps] = useState<string[]>([]);
  ...
  useEffect(() => {
    ...
    setVideoHelpMessage(null);
    setVideoHelpSteps([]);
    setIsAudioOnlyFallback(false);

    const setPlaybackHelp = (message: string, steps: string[], audioOnly = false) => {
      setVideoHelpMessage(message);
      setVideoHelpSteps(steps);
      setIsAudioOnlyFallback(audioOnly);
    };

    const clearPlaybackHelp = () => {
      setVideoHelpMessage(null);
      setVideoHelpSteps([]);
      setIsAudioOnlyFallback(false);
    };

    const scheduleAudioOnlyFrameCheck = () => {
      if (frameCheckTimerRef.current) {
        clearTimeout(frameCheckTimerRef.current);
      }

      frameCheckTimerRef.current = setTimeout(() => {
        if (cancelled || video.ended || video.paused) {
          return;
        }

        // Audio can continue while video frames fail to decode on some devices/codecs.
        const likelyAudioOnly = video.currentTime > 1.2 && video.videoWidth === 0 && video.readyState >= HTMLMediaElement.HAVE_CURRENT_DATA;
        if (likelyAudioOnly) {
          setPlaybackHelp(
            "Audio is playing, but your browser is not rendering video frames.",
            [
              "Tap play/pause once, then play again.",
              "Refresh this page and try once more.",
              "If this continues, try Chrome/Edge/Safari latest version or disable hardware acceleration for this tab.",
              "Open the direct video link in a new tab."
            ],
            true
          );
        }
    ...
    const handleVideoError = () => {
      ...
      if (!mediaError) {
        setPlaybackHelp(
          "This video could not be played in your browser right now.",
          [
            "Refresh the page and try again.",
            "Check your network and try a different browser.",
            "Open the direct video link in a new tab."
          ]
        );
        return;
      ...
      const errorMessageByCode: Record<number, string> = {
        [MediaError.MEDIA_ERR_ABORTED]: "Video playback was interrupted before completion.",
        [MediaError.MEDIA_ERR_NETWORK]: "A network issue interrupted video playback.",
        [MediaError.MEDIA_ERR_DECODE]: "Your browser could load audio, but failed to decode video frames.",
        [MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED]: "This browser could not play the current video format."
      };

      setPlaybackHelp(
        errorMessageByCode[mediaError.code] ?? "Your browser could not render this video.",
        [
          "Refresh this page once.",
          "Try the latest Chrome, Edge, or Safari.",
          "If company policy blocks media codecs, ask to enable standard MP4/H.264 playback.",
          "Open the direct video link in a new tab."
        ],
    ...
    const attemptAutoplay = async () => {
      ...
      try {
        ...
        setPlaybackHelp(
          ...
          [
            "Tap the play button to start video.",
            "If prompted, allow site media playback/autoplay for this website.",
            "If your browser is in data saver or low power mode, disable it for video playback."
          ]
        );
      }
    };
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="overflow-hidden rounded-2xl border border-[var(--shresta-logo-border)] bg-[var(--shresta-logo-surface)] shadow-[0_20px_50px_rgba(47,33,21,0.14)]">
          ...
          <div className="relative aspect-video w-full bg-black">
            ...
            {videoHelpMessage ? (
              <div className="pointer-events-none absolute inset-x-4 bottom-4 rounded-xl border border-amber-300/45 bg-[rgba(28,18,10,0.82)] p-3 text-amber-100 shadow-[0_10px_30px_rgba(0,0,0,0.35)] backdrop-blur-sm sm:inset-x-6 sm:p-4">
                <p className="text-sm font-semibold">{videoHelpMessage}</p>
                <ul className="mt-2 list-disc space-y-1 pl-4 text-xs leading-5 text-amber-50/95 sm:text-sm">
                  {videoHelpSteps.map((step) => (
                    <li key={step}>{step}</li>
                  ))}
                </ul>
                <div className="mt-2 text-xs text-amber-200/90 sm:text-sm">
                  <a
                    className="pointer-events-auto font-semibold underline decoration-amber-300/70 underline-offset-2 hover:text-amber-100"
                  ...
                  {isAudioOnlyFallback ? " if the player continues with audio-only playback." : "."}
                </div>
              </div>
            ) : null}
          </div>
...
function CollectionCard({ collection, index }: { collection: FeaturedCollection; index: number }) {
  ...
  return (
    ...
    >
      <Link className="block" href={`/collections/${collection.slug}`} prefetch={false}>
      <motion.article
...
function ProductTile({
  ...
  return (
    ...
    >
      <Link className="group/card flex h-full flex-col" href={`/products/${product.slug}`} prefetch={false}>
        <motion.div
        ...
        >
          ...
          <ProductImageBadgeRow badgeIcons={product.badgeIcons} badges={product.badges} />
          <ResponsiveMedia
          ...
          <div className="absolute bottom-4 left-4 right-4 translate-y-3 opacity-0 transition duration-300 group-hover:translate-y-0 group-hover:opacity-100">
            <button
              ...
              title={product.stockQuantity === 0 ? "This product is currently out of stock." : undefined}
              onClick={(event) => {
                event.preventDefault();
  ...
  return (
    <footer className="bg-[var(--shresta-logo-bg)] pb-24 pt-12 text-[var(--shresta-logo-muted)] md:pb-0">
      <div className="mx-auto grid max-w-7xl gap-7 px-4 pb-8 sm:px-6 md:grid-cols-2 lg:grid-cols-4">
        ...
        <FooterList title="Categories" links={home.featuredCollections.slice(0, 6).map((collection) => ({ label: collection.title, href: `/collections/${collection.slug}` }))} />
        ...
        <div>
          ...
          <ul className="space-y-3 text-sm text-[var(--shresta-logo-muted)]">
            <li>
              ...
              <a className="transition hover:text-gold-400" href={`mailto:${SUPPORT_EMAIL}`}>{SUPPORT_EMAIL}</a>
            </li>
            <li>
              <span className="block text-[var(--shresta-logo-muted)]">Phone</span>
              <a className="transition hover:text-gold-400" href={`tel:${SUPPORT_PHONE_DIAL}`}>{SUPPORT_PHONE_DISPLAY}</a>
            </li>
            <li>
      ...
      <div className="border-t border-[var(--shresta-logo-border)]">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-6 text-sm text-[var(--shresta-logo-muted)] sm:px-6 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-5">
            <Link className="transition hover:text-gold-400" href="/privacy" prefetch={false}>Privacy Policy</Link>
            <Link className="transition hover:text-gold-400" href="/terms" prefetch={false}>Terms of Service</Link>
            <Link className="transition hover:text-gold-400" href="/cookies" prefetch={false}>Cookie Policy</Link>
          </div>
        </div>
      </div>
    </footer>
  ...
  return (
    <div>
      ...
      <ul className="mt-4 space-y-3">
        {links.map((link) => (
          <li key={link.href}>
            <Link className="text-sm text-[var(--shresta-logo-muted)] transition-colors hover:text-gold-400" href={link.href} prefetch={false}>
              {link.label}
            </Link>
          </li>
        ))}
