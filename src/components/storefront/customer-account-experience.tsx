...
import { fetchCustomerOrder, fetchCustomerOrders, requestCustomerOrderRefund, type CustomerOrderResponse, type CustomerOrderSummary } from "@/features/orders/customer-orders";
...
export function CustomerAccountExperience({ home }: { home: StorefrontHome }) {
  ...
  const [requestingOrderActionNumber, setRequestingOrderActionNumber] = useState<string | null>(null);
  const [checkingOrderNumber, setCheckingOrderNumber] = useState<string | null>(null);
  const [refundFeedbackByOrderNumber, setRefundFeedbackByOrderNumber] = useState<Record<string, string>>({});
  const [refundLockedOrderNumbers, setRefundLockedOrderNumbers] = useState<string[]>([]);
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      const key = refundLockStorageKey(activeCustomerId);
      try {
        const parsed = JSON.parse(window.localStorage.getItem(key) ?? "[]") as unknown;
        if (active && Array.isArray(parsed)) {
          const nextLocks = parsed.filter((value): value is string => typeof value === "string");
          window.requestAnimationFrame(() => {
            if (active) {
              setRefundLockedOrderNumbers(nextLocks);
            }
          });
        }
      } catch {
  ...
  async function handleCheckOrderStatus(orderNumber: string) {
    if (!session) {
      return;
    }

    setCheckingOrderNumber(orderNumber);
    const result = await fetchCustomerOrder(orderNumber);
    ...
    setOrdersState((current) => {
      const currentOrders = current.customerId === session.customerId ? current.orders : [];
      return {
        customerId: session.customerId,
        error: null,
        orders: currentOrders.map((order) => (
          order.orderNumber === orderNumber
            ? {
                ...order,
                customerStageCode: result.order.customerStageCode,
                customerStageIndex: result.order.customerStageIndex,
                customerStageLabel: result.order.customerStageLabel,
                customerStageMeaning: result.order.customerStageMeaning,
                customerStageTerminal: result.order.customerStageTerminal,
                fulfillmentStatus: result.order.fulfillmentStatus,
                orderStatus: result.order.orderStatus,
                paymentStatus: result.order.paymentStatus,
                refundRequestStatus: result.order.refundRequestStatus
              }
            : order
        ))
      };
  ...
  async function handleCancelOrRefund(orderNumber: string) {
    if (!session) {
      return;
    }

    setRequestingOrderActionNumber(orderNumber);
    const result = await requestCustomerOrderRefund(orderNumber, crypto.randomUUID(), "Refund requested by customer from profile orders page.");
    setRequestingOrderActionNumber(null);
    ...
    const responseRefundStatus = normalizedRefundStatus(result.order.refundRequestStatus);
    const effectiveRefundStatus = responseRefundStatus === "NONE" ? "REQUESTED" : responseRefundStatus;

    const updatedLocks = Array.from(new Set([...refundLockedOrderNumbers, orderNumber]));
    setRefundLockedOrderNumbers(updatedLocks);
    if (typeof window !== "undefined") {
      window.localStorage.setItem(refundLockStorageKey(session.customerId), JSON.stringify(updatedLocks));
    }

    setRefundFeedbackByOrderNumber((current) => ({
      ...current,
      [orderNumber]: effectiveRefundStatus === "REQUESTED" || effectiveRefundStatus === "PROCESSING" || effectiveRefundStatus === "SUCCESS"
        ? `Refund request accepted. Current status: ${customerRefundStatusLabel(effectiveRefundStatus)}.`
        : `Refund request submitted. Current status: ${customerRefundStatusLabel(effectiveRefundStatus)}.`
    }));
    ...
    if (refreshed.ok) {
      setOrdersState({
        customerId: session.customerId,
        error: null,
        orders: refreshed.orders.map((order) => (
          order.orderNumber === orderNumber && normalizedRefundStatus(order.refundRequestStatus) === "NONE"
            ? { ...order, refundRequestStatus: effectiveRefundStatus }
            : order
        ))
      });
    } else {
    ...
    setOrderDetailsState((current) => ({
      ...
      detailsByOrderNumber: {
        ...
        [orderNumber]: {
          ...result.order,
          refundRequestStatus: effectiveRefundStatus
        }
      },
      errorByOrderNumber: {
        ...(current.customerId === session.customerId ? current.errorByOrderNumber : {}),
        ...
        <div className="mx-auto max-w-7xl">
          {isLoading ? (
            ...
            <div className="grid gap-6 lg:grid-cols-[380px_minmax(0,1fr)]">
              ...
              <div className="space-y-5">
                ...
                {activeAccountView === "ORDERS" ? (
                  <OrdersPanel
                    checkingOrderNumber={checkingOrderNumber}
                    ...
                    refundLockedOrderNumbers={refundLockedOrderNumbers}
                    refundFeedbackByOrderNumber={refundFeedbackByOrderNumber}
                    requestingOrderActionNumber={requestingOrderActionNumber}
                  />
                ) : (
            ...
            <div className="rounded-2xl border border-[var(--shresta-logo-border)] bg-[var(--shresta-logo-surface)] p-8 text-center shadow-[0_22px_70px_rgba(0,0,0,0.26)]">
              ...
              <Link className="admin-button mt-7 inline-flex" href="/login?next=/account" prefetch={false}>
                Login to account
              </Link>
            </div>
          )}
...
function OrdersPanel({
  ...
  onToggleOrderView,
  orders,
  refundLockedOrderNumbers,
  refundFeedbackByOrderNumber,
  requestingOrderActionNumber
}: {
  checkingOrderNumber: string | null;
  ...
  loading: boolean;
  onCancelOrRefund: (orderNumber: string) => void;
  onCheckOrderStatus: (orderNumber: string) => void;
  onToggleOrderView: (orderNumber: string) => void;
  orders: CustomerOrderSummary[];
  refundLockedOrderNumbers: string[];
  refundFeedbackByOrderNumber: Record<string, string>;
  requestingOrderActionNumber: string | null;
}) {
  return (
    <section className="rounded-2xl border border-[var(--shresta-logo-border)] bg-[var(--shresta-logo-surface)] p-5 shadow-[0_18px_60px_rgba(0,0,0,0.22)]">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        ...
        <Link className="text-sm font-bold text-[var(--gold-600)] hover:text-[var(--gold-500)]" href="/products" prefetch={false}>
          Continue shopping
        </Link>
      </div>
        ...
        <div className="mt-5 space-y-3">
          {orders.map((order) => (
            <article className="rounded-xl border border-[var(--shresta-logo-border)] bg-[var(--shresta-logo-surface)] p-4" key={order.orderNumber}>
              {(() => {
                const refundLocked = refundLockedOrderNumbers.includes(order.orderNumber);
                const effectiveRefundStatus = refundLocked && normalizedRefundStatus(order.refundRequestStatus) === "NONE"
                  ? "REQUESTED"
              ...
              <p className="mt-3 text-xs font-semibold text-[var(--shresta-logo-muted)]">
                Refund status: <span className="text-[var(--gold-600)]">{customerRefundStatusLabel(effectiveRefundStatus)}</span>
              </p>
              <div className="mt-4 flex flex-wrap items-center gap-3">
                ...
                <button
                  className="inline-flex min-h-10 items-center justify-center rounded-full border border-[var(--shresta-logo-border)] px-4 text-sm font-semibold text-[var(--shresta-logo-text)] transition hover:border-[var(--gold-500)] hover:text-[var(--gold-600)] disabled:cursor-not-allowed disabled:opacity-60"
                  disabled={checkingOrderNumber === order.orderNumber}
                  onClick={() => onCheckOrderStatus(order.orderNumber)}
                  title={checkingOrderNumber === order.orderNumber ? "Order status check is already in progress. Please wait." : undefined}
                  type="button"
                >
                  {checkingOrderNumber === order.orderNumber ? "Checking..." : "Check Status"}
                </button>
                <div className="grid gap-1">
                  <button
                    className="inline-flex min-h-10 items-center justify-center rounded-full border border-[var(--gold-500)] px-4 text-sm font-semibold text-[var(--gold-700)] transition hover:bg-[rgba(212,175,55,0.12)] disabled:cursor-not-allowed disabled:opacity-60"
                    disabled={!canCustomerRequestRefund(order, refundLocked) || requestingOrderActionNumber === order.orderNumber}
                    onClick={() => onCancelOrRefund(order.orderNumber)}
                    title={requestingOrderActionNumber === order.orderNumber
                      ? "Refund request is being submitted. Please wait."
                      : !canCustomerRequestRefund(order, refundLocked)
                        ? refundActionUnavailableReason(order, effectiveRefundStatus)
                        : undefined}
                    type="button"
                  >
                    {requestingOrderActionNumber === order.orderNumber ? "Requesting..." : canCustomerRequestRefund(order, refundLocked) ? "Cancel / Refund" : "Refund unavailable"}
                  </button>
                  {!canCustomerRequestRefund(order, refundLocked) ? (
                    <p className="text-xs text-[var(--shresta-logo-muted)]">{refundActionUnavailableReason(order, effectiveRefundStatus)}</p>
                  ) : null}
                  {refundFeedbackByOrderNumber[order.orderNumber] ? (
                    <p className="text-xs text-[var(--gold-700)]">{refundFeedbackByOrderNumber[order.orderNumber]}</p>
                  ) : null}
                </div>
              </div>
...
function canCustomerRequestRefund(order: CustomerOrderSummary, refundLocked = false): boolean {
  const orderStatus = order.orderStatus.trim().toUpperCase();
  const fulfillmentStatus = order.fulfillmentStatus.trim().toUpperCase();
  const paymentStatus = order.paymentStatus.trim().toUpperCase();
  if (refundLocked) {
    return false;
  }
  const refundStatus = normalizedRefundStatus(order.refundRequestStatus);
  if (paymentStatus !== "CAPTURED") {
    return false;
  }
  if (refundStatus === "REQUESTED" || refundStatus === "PROCESSING" || refundStatus === "SUCCESS") {
    return false;
  }
...
function normalizedRefundStatus(status: string | null | undefined): string {
  const normalized = (status ?? "").trim().toUpperCase();
  return normalized || "NONE";
}

function customerRefundStatusLabel(status: string | null | undefined): string {
  const normalized = normalizedRefundStatus(status);
  if (normalized === "NONE") {
...
function refundActionUnavailableReason(order: CustomerOrderSummary, statusOverride?: string): string {
  const paymentStatus = order.paymentStatus.trim().toUpperCase();
  const orderStatus = order.orderStatus.trim().toUpperCase();
  const fulfillmentStatus = order.fulfillmentStatus.trim().toUpperCase();
  const refundStatus = normalizedRefundStatus(statusOverride ?? order.refundRequestStatus);

  if (refundStatus === "REQUESTED") {
    return "Refund request has been accepted and is awaiting processing.";
  }
  if (refundStatus === "PROCESSING") {
    return "Refund has been initiated. Please wait for completion.";
  }
  if (refundStatus === "SUCCESS") {
    return "Refund is already completed for this order.";
  }
  if (paymentStatus !== "CAPTURED") {
    return "Refund is available only after successful payment.";
  }
  if (orderStatus !== "DELIVERED" && fulfillmentStatus !== "DELIVERED") {
    return "Refund is available only after delivery.";
  }
  return "Refund is not available for this order.";
}

function refundLockStorageKey(customerId: string): string {
  return `shresta-refund-locks:${customerId}`;
}
...
function OrderProgressBar({ activeIndex, stages, terminalLabel }: { activeIndex: number | null; stages: string[]; terminalLabel?: string }) {
  ...
  const moveLeftDisabledReason = "You are already at the first stage. Swipe or use Move right to view later stages.";
  const moveRightDisabledReason = "You are already at the last stage. Swipe or use Move left to view earlier stages.";
  ...
  return (
    <div className="mt-4 min-w-0 max-w-full">
      {!shouldWrap ? (
        <div className="mb-2 flex justify-end gap-2">
          <button
            ...
            title={!canScrollLeft ? moveLeftDisabledReason : undefined}
            type="button"
          >
          ...
          <button
            ...
            onClick={() => scrollContainerRef.current?.scrollBy({ left: 220, behavior: "smooth" })}
            title={!canScrollRight ? moveRightDisabledReason : undefined}
            type="button"
          >
            Move right
      ...
      {!shouldWrap && (!canScrollLeft || !canScrollRight) ? (
        <p className="mb-2 text-right text-[0.66rem] font-medium text-[var(--shresta-logo-muted)]">
          {!canScrollLeft ? moveLeftDisabledReason : moveRightDisabledReason}
        </p>
      ) : null}
...
type CustomerMainStageCode = "PAYMENT" | "ORDER_PLACED" | "PACKED" | "ON_THE_WAY" | "DELIVERED" | "REFUNDED" | "CANCELLED";
...
const REFUNDED_STAGE = {
  label: "Refunded",
  meaning: "Your refund has been completed successfully."
};

const STAGE_BY_CODE: Record<Exclude<CustomerMainStageCode, "CANCELLED" | "REFUNDED">, CustomerMainStage> = {
...
function buildOrderProgress(detail: CustomerOrderResponse): {
  ...
  if (stageFromBackend) {
    ...
    if (stageFromBackend === "REFUNDED") {
      return {
        activeIndex: 4,
        completedThroughIndex: 4,
        currentCode: "REFUNDED",
        currentLabel: detail.customerStageLabel || REFUNDED_STAGE.label,
        currentMeaning: detail.customerStageMeaning || REFUNDED_STAGE.meaning,
        stages: CUSTOMER_MAIN_STAGES
      };
    }
  ...
  if (paymentStatus === "REFUNDED") {
    return {
      ...
      completedThroughIndex: 4,
      currentCode: "REFUNDED",
      currentLabel: REFUNDED_STAGE.label,
      currentMeaning: REFUNDED_STAGE.meaning,
      stages: CUSTOMER_MAIN_STAGES
    };
  }
...
function AccountActionCard({
  ...
  return (
    <article className="rounded-2xl border border-[var(--shresta-logo-border)] bg-[var(--shresta-logo-surface)] p-5 shadow-[0_18px_60px_rgba(0,0,0,0.22)]">
      ...
      <Link className="mt-5 inline-flex text-sm font-bold text-[var(--gold-600)] hover:text-[var(--gold-500)]" href={href} prefetch={false}>
        {label}
      </Link>
    </article>
  );
