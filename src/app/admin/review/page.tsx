...
function buildProductDiff(payload: Record<string, unknown>, current: ProductCard): DiffRow[] {
  ...
  const rows: DiffRow[] = [
    ...
    row("Demo Video Asset", current.demoVideoUrl, payload.demoVideoAssetKey),
...
function buildProductImageDiff(): DiffRow[] {
...
function buildProductVideoDiff(payload: Record<string, unknown>): DiffRow[] {
  const assetKey = String(payload.demoVideoAssetKey ?? "");
  return [{ field: "Demo Video Asset", before: "—", after: assetKey === "" ? "(cleared)" : assetKey, changed: true }];
}
...
function buildNewProductRows(payload: Record<string, unknown>): DiffRow[] {
  ...
  return [
    ...
    toRow("Long Description", meta.longDescription ?? payload.longDescription),
...
function buildTestUserCreateRows(payload: Record<string, unknown>): DiffRow[] {
  const toRow = (field: string, v: unknown): DiffRow => ({ field, before: "—", after: fmtVal(v), changed: true });
  return [
    toRow("Display name", payload.displayName),
    toRow("Email", payload.email),
    toRow("Mobile", payload.mobile),
    toRow("Note", payload.note),
  ].filter((r) => r.after !== "—");
}

function buildTestUserDeleteRows(request: AdminChangeRequestResponse): DiffRow[] {
  const toRow = (field: string, v: unknown): DiffRow => ({ field, before: fmtVal(v), after: "(permanently deleted)", changed: true });
  return [
    toRow("Customer ID", request.payload.customerId ?? request.entityKey),
    toRow("Reason", request.payload.reason),
  ];
}
...
function ReviewCard({ request, products }: { request: AdminChangeRequestResponse; products: ProductCard[] }) {
  ...
  if (isProductUpdate) {
    if (request.requestType === "storefront-product-merchandising" && currentProduct) {
      ...
      diffRows = buildProductImageDiff();
  ...
  } else if (request.requestType === "test-user-management") {
    // The OTP is minted at approval time and is never part of any payload — no secret rows here.
    diffRows = request.action === "CREATE"
      ? buildTestUserCreateRows(request.payload)
      : request.action === "DELETE"
        ? buildTestUserDeleteRows(request)
        : diffRows;
  }
  ...
  return (
    <article className="admin-panel rounded-lg p-4">
      ...
      <div className="mt-4 grid gap-3 border-t border-[var(--shresta-logo-border)] pt-4 lg:grid-cols-[1fr_180px_180px]">
        ...
        <AdminActionForm action={approveChangeRequestAction} className="self-end" id={`approve-${request.requestKey}`}>
          ...
          {(request.action === "DELETE" || request.action === "ARCHIVE") ? (
            <ConfirmSubmitButton
              ...
              message={`Approving this will PERMANENTLY ${request.action === "DELETE" ? "delete the asset record and" : "archive the asset and delete the"} object from R2. This cannot be undone. Approve?`}
            >
              Approve
            </ConfirmSubmitButton>
          ) : (
        ...
        <AdminActionForm action={rejectChangeRequestAction} className="self-end">
          <input name="idempotencyKey" type="hidden" value={randomUUID()} />
          <input name="requestKey" type="hidden" value={request.requestKey} />
