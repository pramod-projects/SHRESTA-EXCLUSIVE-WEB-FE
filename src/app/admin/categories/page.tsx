...
import { CategoryMerchandisingEditor } from "@/components/admin/category-merchandising-editor";
...
export default async function AdminCategoriesPage() {
  await requireAdminModuleAccess("categories");
      ...
      <section className="space-y-5">
        {categories.map((family) => (
          <article className="admin-panel rounded-lg p-4" key={family.familyKey}>
            ...
            <details className="admin-surface mt-5 rounded-lg p-4">
              <summary className="cursor-pointer list-none font-serif text-xl font-light text-[var(--shresta-logo-text)]">Product Tags &amp; Colours</summary>
              <p className="mt-2 text-sm text-[var(--shresta-logo-muted)]">
                Manage product tag labels and Lucide icons, plus colour filter options for this category. Changes require approval.
              </p>
              <CategoryMerchandisingEditor familyKey={family.familyKey} idempotencyKey={randomUUID()} metadata={family.metadata} />
            </details>
