...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (!sessionToken) {
    return NextResponse.json({
      success: false,
      data: null,
      error: { code: "CUSTOMER_UNAUTHENTICATED", message: "Login is required to request a refund." }
    }, { status: 401 });
  }
  ...
  if (!idempotencyKey) {
    return NextResponse.json({
      success: false,
      data: null,
      error: { code: "IDEMPOTENCY_KEY_REQUIRED", message: "Idempotency-Key is required for refund request." }
    }, { status: 400 });
  }
  ...
  try {
    const data = await requestApi<Record<string, unknown>>(`/api/v1/customer/orders/${encodeURIComponent(orderNumber)}/refund-request` as `/${string}`, {
    ...
    return NextResponse.json({
      success: false,
      data: null,
      error: { code: "CUSTOMER_ORDER_REFUND_REQUEST_PROXY_FAILED", message: "We couldn't submit your refund request right now. Please try again shortly." }
    }, { status: 503 });
  }
}
