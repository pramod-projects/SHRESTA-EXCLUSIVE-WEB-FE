...
export async function POST(request: Request) {
  ...
  if (!sessionToken) {
    return NextResponse.json({
      success: false,
      data: null,
      error: { code: "CUSTOMER_UNAUTHENTICATED", message: "Login is required before checkout can start." }
    }, { status: 401 });
  }
  ...
  if (!idempotencyKey) {
    return NextResponse.json({
      success: false,
      data: null,
      error: { code: "IDEMPOTENCY_KEY_REQUIRED", message: "Idempotency-Key is required to create a checkout order ID." }
    }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({
      success: false,
      data: null,
      error: { code: "INVALID_JSON", message: "We couldn't read your checkout request. Please refresh and try again." }
    }, { status: 400 });
  }

  try {
    const data = await requestApi<Record<string, unknown>>("/api/v1/customer/orders/draft", {
    ...
    return NextResponse.json({
      success: false,
      data: null,
      error: { code: "CUSTOMER_ORDER_DRAFT_PROXY_FAILED", message: "Checkout isn't available right now. Please try again in a moment." }
    }, { status: 503 });
  }
}
