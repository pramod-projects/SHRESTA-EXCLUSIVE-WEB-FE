...
Render the home page from `GET /api/v1/storefront/home` through `src/features/storefront/storefront-home.ts` and `src/components/storefront/storefront-home-experience.tsx`. The customer shell keeps all data-bearing storefront surfaces backend-owned. Use `ResponsiveMedia` with the one canonical Cloudflare URL and UI-specific `next/image` sizes.
